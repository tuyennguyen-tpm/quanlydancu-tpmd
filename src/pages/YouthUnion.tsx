...
const YouthUnion = () => {
  ...
  const isCanBoChung = currentRole === 'doan_thanh_nien' || currentRole === 'admin' || currentRole === 'to_truong' || currentRole === 'all' || currentRole === 'can_bo_chung';
  ...
  const handlePrint = () => {
    ...
    filteredMembers.forEach((m, idx) => {
      ...
      rowsHtml += `
        <tr>
          ...
          <td style="text-align: center; padding: 8px; border: 1px solid #000;">${genderText}</td>
    ...
    const htmlContent = `
      <html>
        <head>
          <title>DANH SÁCH ĐOÀN VIÊN THANH NIÊN</title>
          <style>
            @media print {
              @page {
                size: A4 portrait;
        ...
        <body>
          ...
          <h2 style="margin-top: 30px; text-align: center;">DANH SÁCH ĐOÀN VIÊN CHI ĐOÀN THANH NIÊN</h2>
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 40px; font-size: 12pt; page-break-inside: avoid;">
            <div style="width: 40%; text-align: center;">
              <p style="font-weight: bold; margin-bottom: 60px;">BÍ THƯ CHI ĐOÀN</p>
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet('DoanVienThanhNien');

      const headers = ['STT', 'Họ và tên', 'Giới tính', 'Ngày sinh', 'Tuổi', 'Điện thoại', 'Nghề nghiệp', 'Địa chỉ cư trú', 'Trạng thái'];
      ...
      headerRow.eachCell((cell) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFDC2626' } // Red for Youth Union
        };
        cell.font = {
          bold: true,
          color: { argb: 'FFFFFFFF' },
      ...
      filteredMembers.forEach((m, idx) => {
        ...
        addedRow.eachCell((cell, colNumber) => {
          ...
          cell.alignment = {
            vertical: 'middle',
            horizontal: colNumber === 1 || colNumber === 3 || colNumber === 4 || colNumber === 5 || colNumber === 6 || colNumber === 9 ? 'center' : 'left'
          };
          cell.border = {
      ...
      link.setAttribute('download', `danh_sach_doan_vien_thanh_nien_${new Date().toISOString().slice(0,10)}.xlsx`);
