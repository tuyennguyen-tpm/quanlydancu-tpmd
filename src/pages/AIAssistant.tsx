...
Yêu cầu bà con nhân dân lưu ý và phối hợp thực hiện nghiêm túc nội dung trên.`;
    ...
    docText = docText
      ...
      .replace(/{tongHoDan}/g, String(households.filter(h => h.status !== 'moved_out').length))
      .replace(/{tongNhanKhau}/g, String(residents.filter(r => r.status !== 'deceased' && r.status !== 'moved_out' && (r.relationship_with_head || '').trim().toLowerCase() !== 'thành viên chuyển đi').length))
